...
public:
    ...
    void draw() {
      ...
      if ((xr > yr) || (xr > zr)) {
        ...
        renderer.push();
        renderer.translate(vec3(0.0,0.0,0.0));
        renderer.scale(vec3(2*sx, 2*sy, 2*sz));
        mesh = m[2];
        renderer.mesh(mesh);
        renderer.pop();

        renderer.push();
        renderer.translate(vec3(0.0,0.0,-0.6));
        renderer.scale(vec3(0.5*sx, 0.5*sy, 0.5*sz));
        renderer.rotate(kPI/.70,vec3(1,0,0));
        mesh = m[3];
        renderer.mesh(mesh);
        renderer.pop();

        renderer.push();
        renderer.translate(vec3(5.0,5.0,5.0));
        renderer.scale(vec3(3*sx, 3*sy, 3*sz));
        mesh = m[2];
        renderer.mesh(mesh);
        renderer.pop();

        renderer.push();
        renderer.translate(vec3(10.0,10.0,0.0));
        renderer.scale(vec3(2*sx, 2*sy, 2*sz));
        mesh = m[2];
        renderer.mesh(mesh);
        renderer.pop();
